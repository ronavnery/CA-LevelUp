<template>
  <section class="offer-filter-wrapper">
    ...
    <div class="type-wrapper">
      <div @click="setType('')" data-value="" ref="type1" class="selected">
        All
      </div>
      <div @click="setType('group', $event)" data-value="group" ref="type2">
        Group
      </div>
      <div @click="setType('single', $event)" data-value="single" ref="type3">
        Single
      </div>
    </div> 
  </section>
  ...
  methods: {
    ...
    setType(selectedValue) {
      ...
      targets.forEach(target => {
        if (target.dataset.value !== selectedValue) target.classList.remove('selected')
